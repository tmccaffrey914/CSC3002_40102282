...
public class DisplayFFStocks {
	...
	private CategoryAxis setUpXAxis() {
		...
		//So that the chart does not display timestamps for each data point
	...
	public LineChart<String, Number> drawChart(String ticker) {
		...
		ResultSet rs = qry.getFFStockBids(ticker);
		...
		animation.getKeyFrames().add(new KeyFrame(Duration.millis(1000/60), new EventHandler<ActionEvent>() {
			@Override public void handle(ActionEvent actionEvent) {	
				try {
					while(rs.next()) {
						String time  = dateFormat.format(rs.getTimestamp("timest"));
						double value = rs.getDouble("price");
						
						//Create a StackPane to add an action to
						...
						sp.setOnMouseClicked(e -> {
							PopUpBox.displayFFNodeInfo(time);
						});
